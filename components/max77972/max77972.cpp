...
esp_err_t MAX77972::set_charge_termination_current(uint16_t iterm_ma) {
    ...
    // Typical LSB is 25mA, max value is 255 (0xFF)
    uint16_t clamped = (iterm_ma > 0xFF * 25) ? (0xFF * 25) : iterm_ma;
    uint8_t value = (clamped / 25);
...
esp_err_t MAX77972::read_design_capacity(uint16_t* capacity_mah) {
    ...
    uint8_t data[2];
    esp_err_t ret = read_registers(Design, data, 2);
    if (ret != ESP_OK) {
        return ret;
    }
...
esp_err_t MAX77972::write_design_capacity(uint16_t capacity_mah) {
    ...
    return write_registers(Design, data, 2);
}
